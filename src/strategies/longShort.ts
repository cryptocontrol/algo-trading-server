...
import BaseStrategy, { IAccount } from './BaseStrategy'
...
export default class RSIStrategy extends BaseStrategy<IRSIStrategyParams> {
  ...
  protected readonly requiredHistory: number = 30 // 30 days by default
  ...
  private constructor (strategy: Strategies, accounts: IAccount[]) {
    super('RSI', strategy, accounts)
  ...
  onCandle (candle: ICandle) {
    ...
    if (rsiVal > this.thresholdHigh) {
      ...
      if (this.trend.persisted && !this.trend.adviced) {
        ...
        // this.advice('short')
        this.accounts.forEach(a => this.advice('short', a))
      ...
      if (this.trend.persisted && !this.trend.adviced) {
        this.trend.adviced = true
        // this.advice('long')
        this.accounts.forEach(a => this.advice('long', a))
      } // else this.advice('do-nothing')
    } else {
      log.debug('in no trend')
      // this.advice('do-nothing')
      this.accounts.forEach(a => this.advice('do-nothing', a))
    }
  }
}
